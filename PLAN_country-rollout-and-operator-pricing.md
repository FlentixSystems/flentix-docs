...
**ALL 9 TASKS BUILT.** Post-build fixes (2026-06-21):
- ✅ **Annual option** now also on the Review step (both prices + saving), reachable on every entry path (deep-link customers no longer miss it). `billing_interval` flows to checkout.
- ✅ **`hub_id` now persisted** on `virtual_office_subscribers` (webhook writes it from session metadata); `resolve-kyc-token` + `submit-kyc-documents` resolve country from `virtual_hubs.country_code` (ES fallback). Fixed a regression where `resolve-kyc-token` read `hub_id` from the `_compat` view (which lacks it) → now reads the base table.
- ✅ **Stripe account hierarchy verified:** platform `acct_1TeFkSLUNJ0dTYxa`, connected/Spacio-Lab `acct_1TjSGaLUnJuGzBr8` (= the hub's `stripe_connect_id`). Setup is correct; fee shows on the platform account (Connect → Application fees), full charge on the connected account. See `GO_LIVE_CHECKLIST.md`.
- ⚠️ **Audit catch:** a blueprint re-proposed `application_fee_amount` in `subscription_data` — REJECTED (Stripe breaks on it). Current code is correct (`application_fee_percent`). Do not apply.
