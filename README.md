...
| [`SPEC_mail-handling.md`](SPEC_mail-handling.md) | Mail scan/forwarding model + billing (deposit/PAYG, postage, IVA) + build status/gaps. Background for the design below. | Architect / engineering |
| [`DESIGN_weekly-mailroom-operations.md`](DESIGN_weekly-mailroom-operations.md) | **Approved design** (2026-07-01) for the daily→weekly mailroom cycle: mail-credit balance + top-ups, hold-by-default per-item disposition, Thu-9pm off-session charge reusing the booking balance-charge system, pay-before-Correos gate. | Architect / engineering |
| [`PLAN_weekly-mailroom-operations.md`](PLAN_weekly-mailroom-operations.md) | Phased implementation plan for the mailroom build (5 phases: ledger + top-up, disposition + digest, weekly charge, admin + gate, refund) — Lovable-build tasks each with a Verify step. | Engineering |
